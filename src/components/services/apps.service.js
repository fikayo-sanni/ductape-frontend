...
export const createApp = async (payload) => {
  try {
    const { token, user_id, app_name, public_key, workspace_id, description } =
      payload;
    return appsClient(`Bearer ${token}`, "application/json").post(
      APPS_CREATE_URL,
      { user_id, app_name, public_key, workspace_id, description }
    );
  } catch (e) {
    throw e;
  }
